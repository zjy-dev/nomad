...
export function readTrustedIngressTokenFromFd(fd) {
  if (!Number.isInteger(fd) || fd < 3) reject('INVALID_TRUSTED_INGRESS_FD', 500);
  const source = Buffer.alloc(33); let offset = 0;
  try {
    const info = fstatSync(fd);
    if (!info.isFIFO() && !info.isSocket()) reject('INVALID_TRUSTED_INGRESS_FD', 500);
...
export function createPairingSession(options) {
  ...
  const trustedIngressToken = validateCapability(options?.trustedIngressToken, 'INVALID_TRUSTED_INGRESS');
  return {
    ...
    validateShellRequest(request) {
      ...
      if (!['none', 'same-origin', 'cross-site'].includes(site)) reject('FETCH_METADATA_REJECTED');
    },
    validateAssetRequest(request) {
      ...
      const valid = mode === 'cors' && ['script', 'font'].includes(destination)
        || mode === 'no-cors' && ['script', 'style', 'image', 'font'].includes(destination);
      if (!valid) reject('FETCH_METADATA_REJECTED');
    },
    async readStart(request) {
      const value = await readPairingJson(request, publicOrigin, trustedIngressToken);
      exactObject(value, ['join_id', 'join_secret', 'device_signing_public_key_sec1', 'device_agreement_public_key_sec1']);
      if (!JOIN_ID.test(value.join_id ?? '')
          ...
          || value.device_signing_public_key_sec1 === value.device_agreement_public_key_sec1) {
    ...
    async readConfirm(request) {
      ...
      const value = await readPairingJson(request, publicOrigin, trustedIngressToken);
      exactObject(value, ['challenge_id', 'expected_epoch', 'device_signing_signature_p1363', 'device_agreement_mac']);
      if (!CHALLENGE_ID.test(value.challenge_id ?? '')
          || !positive(value.expected_epoch)
          || !base64urlBytes(value.device_signing_signature_p1363, 64)
          || !base64urlBytes(value.device_agreement_mac, 32)) reject('PAIRING_REQUEST_INVALID', 400);
      return { capability, body: value };
    },
    async readComplete(request) {
      const capability = requireJoinCookie(request);
      const value = await readPairingJson(request, publicOrigin, trustedIngressToken);
      exactObject(value, ['schema', 'challenge_id', 'expected_epoch', 'device_vault_signature_p1363']);
      if (value.schema !== 'nomad.m3e.pairing.vault-commit.v1'
          || !CHALLENGE_ID.test(value.challenge_id ?? '')
          || !positive(value.expected_epoch)
          || !base64urlBytes(value.device_vault_signature_p1363, 64)) reject('PAIRING_REQUEST_INVALID', 400);
      return { capability, body: value };
    },
    async readAbort(request) {
      ...
      if (value.schema !== 'nomad.m3e.pairing.abort.v1' || !CHALLENGE_ID.test(value.challenge_id ?? '') || !positive(value.expected_epoch)) reject('PAIRING_REQUEST_INVALID', 400);
      return { capability, body: value };
    },
    cookieFor(capability, maxAgeSeconds) {
      validateCapability(capability, 'PAIRING_CAPABILITY_INVALID');
      if (!Number.isSafeInteger(maxAgeSeconds) || maxAgeSeconds < 1 || maxAgeSeconds > MAX_JOIN_COOKIE_AGE_SECONDS) reject('PAIRING_SESSION_EXPIRED', 410);
      return serializeCookie(capability, maxAgeSeconds);
    },
    clearCookie() { return serializeCookie('', 0); },
  };
}
...
export function validateDesktopCreate(value) {
  ...
  if (value.schema !== 'nomad.m3e.pairing.create.v1') reject('PAIRING_REQUEST_INVALID', 400);
  return value;
}

export function validateDesktopApprove(value) {
  exactObject(value, ['schema', 'join_id', 'challenge_id', 'expected_epoch', 'comparison_code']);
  if (value.schema !== 'nomad.m3e.pairing.desktop-approve.v1'
      || !JOIN_ID.test(value.join_id ?? '')
      || !CHALLENGE_ID.test(value.challenge_id ?? '')
      || !positive(value.expected_epoch)
      || !COMPARISON_CODE.test(value.comparison_code ?? '')) reject('PAIRING_REQUEST_INVALID', 400);
  return value;
}

export function validateDesktopCancel(value) {
  exactObject(value, ['schema', 'join_id']);
  if (value.schema !== 'nomad.m3e.pairing.cancel.v1' || !JOIN_ID.test(value.join_id ?? '')) reject('PAIRING_REQUEST_INVALID', 400);
  return value;
}

export function validateDesktopStatus(value) {
  exactObject(value, ['schema', 'join_id']);
  if (value.schema !== 'nomad.m3e.pairing.status.v1' || !JOIN_ID.test(value.join_id ?? '')) reject('PAIRING_REQUEST_INVALID', 400);
  return value;
}
...
export function validateDesktopRevoke(value) {
  ...
  if (!DEVICE_ALIAS.test(value.device_alias ?? '') || !positive(value.expected_epoch)) reject('PAIRING_REQUEST_INVALID', 400);
  return value;
}
...
export async function readDesktopJson(request, expectedOrigin, csrfToken) {
  ...
  exactSecretHeader(request, 'x-nomad-csrf', validateCapability(csrfToken, 'CSRF_REJECTED'), 'CSRF_REJECTED');
...
async function readStrictBody(request) {
  ...
  if (rawHeaderValues(request, 'content-encoding').length !== 0
      || rawHeaderValues(request, 'transfer-encoding').length !== 0) reject('PAIRING_FRAMING_INVALID', 400);
  const lengths = rawHeaderValues(request, 'content-length');
  if (lengths.length !== 1 || !/^[1-9][0-9]*$/.test(lengths[0])) reject('PAIRING_FRAMING_INVALID', 400);
  const declared = Number(lengths[0]);
  if (!Number.isSafeInteger(declared) || declared > MAX_PAIRING_REQUEST_BYTES) reject('PAIRING_FRAMING_INVALID', 400);
  ...
  try { return strictJson(utf8.decode(Buffer.concat(chunks))); }
  catch { reject('PAIRING_JSON_INVALID', 400); }
}

function requireTrustedIngress(request, origin, token) {
  if (!isLoopbackAddress(request.socket?.remoteAddress)) reject('TRUSTED_INGRESS_REQUIRED');
  requireHost(request, origin);
  if (rawHeaderValues(request, 'forwarded').length !== 0) reject('TRUSTED_INGRESS_REQUIRED');
  exactHeader(request, 'x-forwarded-proto', 'https', 'TRUSTED_INGRESS_REQUIRED');
  exactHeader(request, 'x-forwarded-host', origin.host, 'TRUSTED_INGRESS_REQUIRED');
...
function validateEmptyRequest(request) {
  ...
  if (lengths.length === 1 && lengths[0] !== '0') reject('PAIRING_FRAMING_INVALID', 400);
}
...
function parseHttpsOrigin(value) {
  let origin;
  try { origin = new URL(value); } catch { reject('INVALID_PUBLIC_ORIGIN', 500); }
  if (origin.protocol !== 'https:' || origin.username || origin.password || origin.pathname !== '/' || origin.search || origin.hash) reject('INVALID_PUBLIC_ORIGIN', 500);
  return origin;
}

function parseLoopbackOrigin(value) {
  let origin;
  try { origin = new URL(value); } catch { reject('INVALID_DESKTOP_ORIGIN', 500); }
  if (origin.protocol !== 'http:' || origin.hostname !== '127.0.0.1' || origin.username || origin.password || origin.pathname !== '/' || origin.search || origin.hash || !origin.port) reject('INVALID_DESKTOP_ORIGIN', 500);
  return origin;
}

function requireHost(request, origin) { exactHeader(request, 'host', origin.host, 'ORIGIN_REJECTED'); }
function exactHeader(request, name, expected, code = 'FETCH_METADATA_REJECTED') {
...
function optionalExactHeader(request, name, expected) {
  const values = rawHeaderValues(request, name);
  if (values.length > 1 || values.length === 1 && values[0] !== expected) reject('ORIGIN_REJECTED');
}
...
function exactSecretHeader(request, name, expected, code) {
  ...
  const actual = Buffer.from(values[0]); const wanted = Buffer.from(expected);
  if (actual.length !== wanted.length || !timingSafeEqual(actual, wanted)) reject(code);
}
...
function isLoopbackAddress(value) { return value === '127.0.0.1' || value === '::1' || value === '::ffff:127.0.0.1'; }
function validateCapability(value, code) {
  if (!base64urlBytes(value, 32)) reject(code, code.startsWith('INVALID_') ? 500 : 401);
  return value;
}
...
function exactObject(value, keys) {
  if (!value || typeof value !== 'object' || Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype) reject('PAIRING_REQUEST_INVALID', 400);
  const actual = Object.keys(value).sort(); const expected = [...keys].sort();
  if (actual.length !== expected.length || actual.some((key, index) => key !== expected[index])) reject('PAIRING_REQUEST_INVALID', 400);
}
function strictJson(raw) {
  let index = 0; let nodes = 0;
  const ws = () => { while (' \t\r\n'.includes(raw[index] ?? '!')) index += 1; };
  const value = (depth) => { ws(); if (++nodes > 512 || depth > 8) throw new Error('budget'); const char = raw[index]; if (char === '{') return objectValue(depth + 1); if (char === '[') return arrayValue(depth + 1); if (char === '"') return stringValue(); for (const pair of [['true', true], ['false', false], ['null', null]]) if (raw.startsWith(pair[0], index)) { index += pair[0].length; return pair[1]; } const match = raw.slice(index).match(/^-?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?(?:[eE][+-]?[0-9]+)?/); if (!match) throw new Error('value'); index += match[0].length; const number = Number(match[0]); if (!Number.isFinite(number)) throw new Error('number'); return number; };
...
function reject(code, statusCode = 403) { throw new PairingSessionError(code, statusCode); }
