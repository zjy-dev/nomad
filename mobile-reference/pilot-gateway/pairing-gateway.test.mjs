...
const KEY_A = Buffer.concat([Buffer.from([4]), Buffer.alloc(64, 1)]).toString(
  "base64url",
);
...
function fakeHost() {
  ...
  return {
    ...
    async getCurrent() {
      this.calls.push(["projection-current"]);
      return envelope;
    },
    async getStream(sequence) {
      this.calls.push(["projection-stream", sequence]);
      return null;
    },
    async getCommandCapability() {
      this.calls.push(["command-capability"]);
      if (this.failCommandCapability)
        throw new Error("COMMAND_CAPABILITY_UNAVAILABLE");
      return {
        schema: "nomad.product-host.command-capability.v1",
        capability_id: "capability_00000001",
        snapshot_seq: envelope.snapshot_seq,
        snapshot_digest: envelope.digest,
        next_command_seq: 1,
    ...
    async postCommand(body) {
      this.calls.push(["command", body]);
      return {
        schema: "nomad.product-host.command-receipt.v1",
        receipt_id: "receipt_00000001",
        request_id: body.request_id,
        action: body.action,
        snapshot_seq: body.expected_snapshot_seq,
        snapshot_digest: body.expected_snapshot_digest,
    ...
    async createPairing(body) {
      this.calls.push(["create", body]);
      return {
        schema: "nomad.m3e.pairing.created.v1",
        join_id: JOIN_ID,
        join_secret: Buffer.alloc(32, 4).toString("base64url"),
        expires_at: "2026-08-28T00:02:00Z",
      };
    },
    async approvePairing(body) {
      this.calls.push(["approve", body]);
    },
    async cancelPairing(body) {
      this.calls.push(["cancel", body]);
    },
    async getPairingStatus(body) {
    ...
    async getCurrentDevice() {
      ...
      return {
        ...
        principal_alias: "principal_00000001",
        paired: false,
        device: null,
      };
    },
    async revokeDevice(body) {
      this.calls.push(["revoke", body]);
      return {
        schema: "nomad.product-host.device-revoke.v1",
        principal_alias: "principal_00000001",
        device_alias: body.device_alias,
        status: "revoked",
        prior_epoch: body.expected_epoch,
        revoked_epoch: body.expected_epoch + 1,
      };
    },
    async startPairing(body) {
      this.calls.push(["start", body]);
      return {
        schema: "nomad.m3e.pairing.host-start.v1",
        join_cookie_capability: COOKIE,
        join_cookie_max_age_seconds: 87,
        browser_start: {
          schema: "nomad.m3e.pairing.start-response.v1",
          challenge_id: CHALLENGE_ID,
          challenge_bytes_b64: Buffer.alloc(32, 5).toString("base64url"),
          prospective_epoch: 1,
          host_signing_public_key_sec1: KEY_A,
          host_agreement_public_key_sec1: KEY_B,
          issued_at: "2026-08-28T00:00:00Z",
          expires_at: "2026-08-28T00:01:27Z",
        },
      };
    },
    async confirmPairing(capability, body) {
      this.calls.push(["confirm", capability, body]);
      return {
        schema: "nomad.m3e.pairing.confirm-response.v1",
        signed_provisioning_bundle: { safe: true },
      };
    },
    async completePairing(capability, body) {
      this.calls.push(["complete", capability, body]);
      return {
        schema: "nomad.m3e.pairing.complete-response.v1",
        device_alias: "device-12345678",
        pairing_epoch: 1,
      };
    },
    async abortPairing(capability, body) {
      this.calls.push(["abort", capability, body]);
    },
  };
}
...
async function gateway(routeTable, host, distDir) {
  ...
  if (routeTable === "desktop") {
    const path = join(
      mkdtempSync(join(tmpdir(), "nomad-desktop-combined-")),
      "state.sqlite3",
    );
...
function productEnvelope() {
  const value = {
    ...
    host_instance_id: `host-${"2".repeat(32)}`,
    snapshot_seq: 1,
    digest: "sha256:placeholder",
    snapshot: {
      session_alias: `sess-${"3".repeat(32)}`,
...
test("desktop route table composes official projection, commands, static and pairing admin while hiding the standalone join secret", async () => {
  ...
  writeFileSync(
    ...
    "<!doctype html><title>Nomad desktop</title>",
  );
  const host = fakeHost();
  ...
  try {
    ...
    assert.deepEqual(createdBody, {
      ...
      join_id: JOIN_ID,
      join_url: `${PUBLIC_ORIGIN}/j/${JOIN_ID}#${Buffer.alloc(32, 4).toString("base64url")}`,
      expires_at: "2026-08-28T00:02:00Z",
    });
    assert.equal(Object.hasOwn(createdBody, "join_secret"), false);
    assert.equal(
      Object.keys(createdBody).sort().join(","),
      "expires_at,join_id,join_url,schema",
    );
    ...
    assert.equal(
      (
        await raw(running.port, "/api/desktop/pairing/approve", {
          method: "POST",
          headers: desktopHeaders(running.origin),
    ...
    assert.equal(
      (
        await raw(running.port, "/api/desktop/pairing/cancel", {
          method: "POST",
          headers: desktopHeaders(running.origin),
    ...
    const status = await raw(running.port, "/api/desktop/pairing/status", {
      method: "POST",
      headers: desktopHeaders(running.origin),
    ...
    const current = await raw(running.port, "/api/desktop/devices/current", {
      method: "POST",
      headers: desktopHeaders(running.origin),
      body: "{}",
    });
    ...
    assert.equal(
      JSON.parse(current.text).schema,
      "nomad.product-host.device-current.v1",
    );
    const revokeRequest = {
      device_alias: "device-12345678",
      expected_epoch: 1,
    };
    const revoke = await raw(running.port, "/api/desktop/devices/revoke", {
      method: "POST",
      headers: desktopHeaders(running.origin),
    ...
    assert.match(staticShell.text, /Nomad desktop/);
    const projection = await raw(running.port, "/api/alpha/session");
    assert.equal(projection.status, 200);
    assert.equal(JSON.parse(projection.text).schema, "nomad.alpha.readonly.v1");
    ...
    const command = await raw(running.port, "/api/commands", {
      method: "POST",
      headers: {
        Host: new URL(running.origin).host,
        Origin: running.origin,
        "Sec-Fetch-Site": "same-origin",
        "Sec-Fetch-Mode": "cors",
        "X-Nomad-CSRF": capabilityBody.csrf_token,
        "Content-Type": "application/json",
      },
    ...
    assert.equal(
      JSON.parse(command.text).schema,
    ...
    for (const path of [
      ...
      "/api/pairing/join/complete",
      "/api/pairing/join/abort",
    ]) {
      const result = await raw(running.port, path, {
        method: path.startsWith("/j/") ? "GET" : "POST",
        headers: desktopHeaders(running.origin),
        body: path.startsWith("/j/") ? "" : "{}",
      });
      assert.equal(result.status, 404);
    }
    assert.deepEqual(host.calls.slice(6), [
      ["projection-stream", 1],
      ["projection-current"],
      ["command-capability"],
      ["command", commandBody],
...
test("desktop bootstrap rejects wrong origin and join route table does not expose it", async () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(wrongOrigin.text), {
...
test("desktop pairing bootstrap is independent from command capability availability", async () => {
  const directory = mkdtempSync(
    join(tmpdir(), "nomad-desktop-capability-decouple-"),
  );
  writeFileSync(
    join(directory, "index.html"),
    "<!doctype html><title>Nomad desktop</title>",
  ...
  try {
    ...
    assert.equal(created.status, 200);
    assert.deepEqual(
      host.calls.filter((entry) => entry[0] === "create"),
      [["create", createRequest]],
    );
  } finally {
    await running.close();
...
test("join route table strips Host cookie capability, emits exact cookie, and has no desktop admin routes", async () => {
  ...
  writeFileSync(
    join(directory, "index.html"),
    '<!doctype html><title>Nomad pair</title><script src="/assets/pair.js"></script>',
  );
  const host = fakeHost();
  const running = await gateway("join", host, directory);
  try {
    const shell = await raw(running.port, `/j/${JOIN_ID}`, {
      headers: {
        ...joinHeaders(),
        Origin: undefined,
    ...
    assert.equal(shell.text.includes(COOKIE), false);

    const startBody = {
      join_id: JOIN_ID,
      join_secret: Buffer.alloc(32, 7).toString("base64url"),
      device_signing_public_key_sec1: KEY_A,
      device_agreement_public_key_sec1: KEY_B,
    };
    const start = await raw(running.port, "/api/pairing/join/start", {
      method: "POST",
      headers: joinHeaders(),
    ...
    assert.equal(
      ...
      `__Host-nomad-join=${COOKIE}; Secure; HttpOnly; SameSite=Strict; Path=/; Max-Age=87`,
    );
    assert.equal(start.text.includes(COOKIE), false);
    ...
    const confirmBody = {
      challenge_id: CHALLENGE_ID,
      expected_epoch: 1,
      device_signing_signature_p1363: Buffer.alloc(64, 6).toString("base64url"),
      device_agreement_mac: Buffer.alloc(32, 7).toString("base64url"),
    };
    const confirm = await raw(running.port, "/api/pairing/join/confirm", {
      method: "POST",
      headers: joinHeaders({ Cookie: `__Host-nomad-join=${COOKIE}` }),
      body: JSON.stringify(confirmBody),
    });
    assert.equal(confirm.status, 200);
    assert.equal(confirm.headers["set-cookie"], undefined);
    assert.equal(confirm.text.includes(COOKIE), false);

    const completeBody = {
      schema: "nomad.m3e.pairing.vault-commit.v1",
      challenge_id: CHALLENGE_ID,
      expected_epoch: 1,
      device_vault_signature_p1363: Buffer.alloc(64, 8).toString("base64url"),
    };
    const complete = await raw(running.port, "/api/pairing/join/complete", {
      method: "POST",
      headers: joinHeaders({ Cookie: `__Host-nomad-join=${COOKIE}` }),
    ...
    assert.equal(
      complete.headers["set-cookie"][0],
      "__Host-nomad-join=; Secure; HttpOnly; SameSite=Strict; Path=/; Max-Age=0",
    );
    assert.deepEqual(host.calls, [
      ["start", startBody],
      ["confirm", COOKIE, confirmBody],
      ["complete", COOKIE, completeBody],
    ]);

    for (const path of [
      "/api/desktop/pairing/create",
      "/api/desktop/pairing/approve",
      "/api/desktop/pairing/cancel",
      "/api/desktop/pairing/status",
      "/api/desktop/devices/current",
...
test("join listener rejects ordinary HTTP before Host and abort clears cookie only after Host success", async () => {
  const host = fakeHost();
  const running = await gateway("join", host, "/missing");
  try {
    ...
    const untrusted = await raw(running.port, "/api/pairing/join/start", {
      method: "POST",
      headers: { ...joinHeaders(), "X-Nomad-Trusted-Ingress": undefined },
      body,
    });
    ...
    const abort = await raw(running.port, "/api/pairing/join/abort", {
      method: "POST",
      headers: joinHeaders({ Cookie: `__Host-nomad-join=${COOKIE}` }),
...
test("CLI route-table contract requires explicit HTTPS public origin for desktop and join, while only join needs the private ingress FD", () => {
  ...
  const base = [
    "--mode",
    "official-agent-local",
    "--route-table",
    "join",
    "--product-host-socket",
    "/private/tmp/product-host.sock",
    ...identity,
    "--command-key-fd",
    "11",
    "--public-origin",
    ...
    () =>
      parseArgs(
        base.filter(
          (value, index) =>
            !(
              value === "--trusted-ingress-fd" ||
              base[index - 1] === "--trusted-ingress-fd"
            ),
        ),
        {},
      ),
  ...
  assert.throws(
    () => parseArgs([...base, "--state-db", "/tmp/forbidden.db"], {}),
    /must not use/,
  );
  const desktop = [
    "--mode",
...
test("desktop create fails closed without configured public origin before calling Host", async () => {
  ...
  const store = new AlphaStore(
    join(
      mkdtempSync(join(tmpdir(), "nomad-desktop-no-origin-")),
      "state.sqlite3",
    ),
  );
  const server = createServer(
  ...
  try {
    const result = await raw(port, "/api/desktop/pairing/create", {
      method: "POST",
      headers: desktopHeaders(origin),
      body: JSON.stringify({ schema: "nomad.m3e.pairing.create.v1" }),
    });
    assert.equal(result.status, 503);
    assert.deepEqual(JSON.parse(result.text), {
      error: "PAIRING_PUBLIC_ORIGIN_REQUIRED",
    });
    assert.deepEqual(host.calls, []);
  } finally {
    server.close();
