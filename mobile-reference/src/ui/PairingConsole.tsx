...
export function PairingConsole({ client }: PairingConsoleProps) {
  const [device, setDevice] = useState<DesktopPairingCurrentDevice | null>(null);
  const [join, setJoin] = useState<ActiveJoinState | null>(null);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [revokeResult, setRevokeResult] = useState<DesktopPairingRevokeResult | null>(null);
  ...
  useEffect(() => {
    ...
    const schedule = (status: DesktopPairingJoinStatus) => {
      ...
      if (isTerminal(status.state)) {
        if (status.state === 'active') {
          void client.getCurrentDevice().then((currentDevice) => {
    ...
    const tick = async () => {
      try {
        ...
        if (!active) return;
        setJoin((current) => current && current.created.join_id === status.join_id
          ? { ...current, status }
          : current);
        if (status.state === 'cancelled' || status.state === 'expired' || status.state === 'compensated') {
          setJoin(null);
          setBusy(null);
  ...
  const expiresAt = join?.status?.expires_at ?? join?.created.expires_at ?? null;
  const comparisonCode = join?.status?.comparison_code ?? null;
  const statusLabel = join?.status ? describeStatus(join.status.state) : 'Waiting for your phone to open the one-time link.';
  ...
  async function handleApprove() {
    if (!join?.status?.challenge_id || !join.status.expected_epoch || !join.status.comparison_code) return;
    setBusy('approve');
    setError(null);
    try {
  ...
  return (
    <section className="pairing-console section" aria-labelledby="pairing-console-title">
      <div className="section-header">
        <h2 className="section-title" id="pairing-console-title">Remote Pairing</h2>
        <span>{device?.paired ? 'Phone paired' : join ? 'Waiting to pair phone' : 'No phone paired yet'}</span>
      </div>
      ...
      <div className="pairing-console__frame">
        ...
        {loading && <div className="command-status" role="status">Loading desktop pairing state…</div>}
        {error && <div className="perm-block" role="alert" aria-live="assertive"><strong>Pairing blocked</strong><div>{error}</div></div>}
        {revokeResult && <div className="command-status" role="status" aria-live="polite">Phone access {revokeResult.status === 'revoked' ? 'was removed immediately.' : 'was already revoked earlier.'}</div>}
        ...
        {join && (
          <div className="pairing-card" data-testid="pairing-join-card">
            ...
            <div className="pairing-url-block">
              ...
              <textarea id="pairing-join-url" readOnly value={joinUrl ?? ''} aria-label="Pairing link" />
            </div>
            <div className="pairing-code-block" aria-live="polite" aria-atomic="true">
              <span className="eyebrow">Comparison Code</span>
              <strong>{comparisonCode ?? '••••••'}</strong>
              <small>{comparisonCode ? 'Confirm only if this matches your phone.' : 'Waiting for your phone to open the one-time link.'}</small>
            </div>
            <div className="pairing-status-line">{statusLabel}</div>
            <div className="hero-actions">
              <button className="btn btn--ghost" onClick={handleCancel} disabled={busy !== null || isTerminal(join.status?.state ?? 'created')}>Cancel</button>
              {readyToApprove && <button className="btn btn--primary" onClick={handleApprove} disabled={busy !== null}>Codes match</button>}
            </div>
          </div>
        )}
        ...
        {!join && !device?.paired && !loading && (
          <div className="pairing-card">
            ...
            <p className="pairing-copy">Create a one-time pairing link and confirm the same 6-digit code on both screens.</p>
            <div className="hero-actions">
              <button ref={pairButtonRef} className="btn btn--primary" onClick={handleCreate} disabled={busy !== null}>Pair phone</button>
            </div>
          </div>
        )}
        ...
        {device?.paired && device.device && (
          <div className="pairing-card" data-testid="paired-device-card">
            ...
            <dl className="pairing-facts">
              <div><dt>Device Alias</dt><dd>{device.device.device_alias}</dd></div>
              <div><dt>Epoch</dt><dd>{device.device.pairing_epoch}</dd></div>
            </dl>
            {!confirmRevoke ? (
              <div className="hero-actions">
                <button className="btn btn--danger-secondary" onClick={() => setConfirmRevoke(true)} disabled={busy !== null}>Revoke phone</button>
              </div>
            ) : (
              <div className="pairing-inline-warning" role="dialog" aria-modal="false" aria-labelledby="pairing-revoke-title">
                <p id="pairing-revoke-title" className="sr-only">Confirm revoke phone</p>
                <p>This removes this phone&apos;s access immediately. You can pair again later.</p>
                <div className="hero-actions">
                  <button className="btn btn--ghost" onClick={() => setConfirmRevoke(false)} disabled={busy !== null}>Keep access</button>
                  <button ref={revokeNowRef} className="btn btn--danger" onClick={handleRevoke} disabled={busy !== null}>Revoke now</button>
                </div>
              </div>
            )}
          </div>
...
function CountdownBadge({ expiresAt }: { expiresAt: string }) {
  ...
  return (
    <span className={`countdown-badge ${seconds === 0 ? 'countdown-badge--expired' : ''}`}>
      {seconds === 0 ? 'Expired' : `${minutes}:${remainder.toString().padStart(2, '0')}`}
    </span>
  );
}

function isTerminal(state: DesktopPairingJoinStatus['state']): boolean {
  return ['active', 'cancelled', 'expired', 'compensated', 'revoked'].includes(state);
}
...
function messageFromError(reason: unknown): string {
  if (reason instanceof DesktopPairingClientError) {
    ...
    if (reason.code === 'PAIRING_HTTP_ERROR') return 'Desktop pairing request was not accepted by the local Gateway.';
    if (reason.code === 'PAIRING_INVALID_RESPONSE') return 'Desktop pairing response is incompatible.';
  }
  return reason instanceof Error ? reason.message : 'Desktop pairing is unavailable.';
}
