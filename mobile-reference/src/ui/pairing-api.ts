...
const LIFECYCLE_ROUTE_ERROR_CODES = ["REMOTE_UNINSTALL_REVOKE_REQUIRED"] as const;

export function createDesktopPairingClient(
  options: DesktopPairingClientOptions,
  ...
  async function readJson(
    path: string,
    init: RequestInit,
    csrfErrorCode: string | null = null,
    ...
    try {
      ...
      throw new DesktopPairingClientError(
        ...
        "Desktop pairing request failed before an authoritative response was received.",
      );
    }
    if (!response.ok) {
      if (csrfErrorCode !== null) {
        const errorCode = await readErrorCode(response.clone());
        if (errorCode === csrfErrorCode) {
          throw new DesktopPairingClientError(
            errorCode,
            "Desktop pairing CSRF protection rejected the current token.",
          );
        }
      }
      const allowedErrorCode = await readAllowlistedErrorCode(
        response.clone(),
        allowedErrorCodes,
      );
      if (allowedErrorCode !== null) {
        throw new DesktopPairingClientError(
          allowedErrorCode,
          "Desktop pairing request was rejected by the local Gateway.",
        );
      }
      throw new DesktopPairingClientError(
        "PAIRING_HTTP_ERROR",
        `Desktop pairing request failed with HTTP ${response.status}.`,
      );
    }
    ...
    if (contentType === null || !contentType.startsWith("application/json")) {
      throw new DesktopPairingClientError(
        ...
        "Desktop pairing response content type is incompatible.",
      );
    }
    try {
      ...
      throw new DesktopPairingClientError(
        "PAIRING_INVALID_RESPONSE",
        "Desktop pairing response JSON is invalid.",
      );
    }
  }
  ...
  async function bootstrapCsrfToken(): Promise<string | null> {
    ...
    csrfTokenFlight = (async () => {
      ...
      if (
        raw.schema !== "nomad.gateway.desktop-security.v1" ||
        !opaque(raw.csrf_token)
      )
        invalid();
  ...
  async function withCsrfRetry<T>(
    request: (csrf: string) => Promise<T>,
  ): Promise<T> {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      const csrf = await bootstrapCsrfToken();
      if (!opaque(csrf)) {
        throw new DesktopPairingClientError(
          "PAIRING_CSRF_UNAVAILABLE",
          "Desktop pairing controls are unavailable until the local Host issues a same-origin CSRF token.",
        );
      }
      try {
        ...
        if (
          error instanceof DesktopPairingClientError &&
...
function decodeCurrentDevice(value: unknown): DesktopPairingCurrentDevice {
  ...
  if (
    raw.schema !== "nomad.product-host.device-current.v1" ||
    typeof raw.principal_alias !== "string" ||
    typeof raw.paired !== "boolean"
  )
    invalid();
  if (raw.device === null) {
  ...
  if (
    typeof device.device_alias !== "string" ||
    !positiveInt(device.pairing_epoch)
  )
    invalid();
  return {
    schema: raw.schema,
...
function decodeCreatedJoin(value: unknown): DesktopPairingCreatedJoin {
  ...
  if (
    raw.schema !== "nomad.m3e.pairing.desktop-created.v1" ||
    !joinId(raw.join_id) ||
    !timestamp(raw.expires_at) ||
    !joinUrl(raw.join_url, raw.join_id)
  )
    invalid();
  return {
    schema: raw.schema,
...
function decodeJoinStatus(value: unknown): DesktopPairingJoinStatus {
  ...
  if (
    raw.schema !== "nomad.m3e.pairing.status-response.v1" ||
    !joinId(raw.join_id) ||
    !joinState(raw.state) ||
    !timestamp(raw.expires_at)
  )
  ...
  if (raw.expected_epoch !== null && !positiveInt(raw.expected_epoch))
    invalid();
  if (
    raw.comparison_code !== null &&
    (typeof raw.comparison_code !== "string" ||
      !/^[0-9]{6}$/.test(raw.comparison_code))
  )
    invalid();
  return {
    schema: raw.schema,
...
function decodeRevoke(value: unknown): DesktopPairingRevokeResult {
  ...
  if (
    raw.schema !== "nomad.product-host.device-revoke.v1" ||
    typeof raw.principal_alias !== "string" ||
    typeof raw.device_alias !== "string" ||
    !["revoked", "already_revoked"].includes(String(raw.status)) ||
    !positiveInt(raw.revoked_epoch)
  )
    invalid();
...
async function readAllowlistedErrorCode(
  ...
  if (contentType === null || !contentType.startsWith("application/json"))
    return null;
  try {
    const raw = exactObject(await response.json(), ["error"]);
    const code = raw.error;
    if (
      typeof code !== "string" ||
      !allowedCodes.includes(code as (typeof allowedCodes)[number])
    ) {
      return null;
    }
    return code;
...
function invalid(): never {
  throw new DesktopPairingClientError(
    "PAIRING_INVALID_RESPONSE",
    "Desktop pairing response is incompatible.",
  );
}
