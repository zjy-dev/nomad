...
import { BrowserVaultError, type BrowserVaultSession } from '../remote/browser-vault';
import type { PairingJoinStartResult } from '../remote/pairing-client';
import { RemoteSessionError, type RemoteSessionPort as RuntimeRemoteSessionPort } from '../remote/paired-session';
import { PairingConsole } from './PairingConsole';
import { PhonePairingScreen, type PhonePairingClientPort, type PhoneVaultPort } from './PhonePairingScreen';
...
describe('PairingConsole', () => {
  it('creates a one-time join, shows the link, polls status, and exposes desktop approval', async () => {
    ...
    const client: DesktopPairingClient = {
      ...
      getJoinStatus: vi.fn()
        ...
        .mockImplementationOnce(async () => joinStatus(join.join_id, 'started_awaiting_desktop_approval')),
      approveJoin: vi.fn(async () => {}),
      cancelJoin: vi.fn(async () => {}),
      revokeDevice: vi.fn(async () => { throw new Error('unused'); }) as DesktopPairingClient['revokeDevice'],
    };
    ...
    await act(async () => {
      pendingStatus.resolve(joinStatus(join.join_id, 'started_awaiting_desktop_approval'));
      await pendingStatus.promise;
    });

    expect(output.textContent).toContain('Scan or open on your phone. Confirm only if both codes match.');
    expect(output.textContent).toContain('042913');
    expect((output.querySelector('textarea[aria-label="Pairing link"]') as HTMLTextAreaElement).value).toBe(join.join_url);
  ...
  it('shows the active paired phone and requires explicit revoke confirmation', async () => {
    ...
    expect(output.textContent).toContain("This removes this phone's access immediately. You can pair again later.");
    ...
    expect(output.textContent).toContain('Phone access was removed immediately.');
  });
});

describe('PhonePairingScreen', () => {
  it('starts from the join route, verifies the remote session, and mounts the real shell', async () => {
    window.history.replaceState(null, '', '/j/join-1234567890abcdef1234567890abcdef');
    ...
    const vault: PhoneVaultPort = {
      restorePairedDevice: vi.fn(async () => { throw new BrowserVaultError('BROWSER_VAULT_EMPTY', 'empty'); }),
    };
    ...
    expect(output.textContent).toContain('Only confirm if this code matches your Mac');
    expect(output.textContent).toContain('042913');
  ...
  it('shows connecting while restore succeeds but remote verification is still pending', async () => {
    ...
    const pairingClient: PhonePairingClientPort = {
      ...
      confirm: vi.fn(async () => ({ comparisonCode: '042913', session: browserSession() })),
      cancelPending: vi.fn(),
      abortPending: vi.fn(async () => {}),
    };
    const vault: PhoneVaultPort = {
  ...
  it('shows revoked when remote verification fails closed with device revocation', async () => {
    ...
    const output = await render(
      <PhonePairingScreen
        pairingClient={pairingClient}
        vault={vault}
        remoteSessionFactory={vi.fn(async () => { throw new RemoteSessionError('DEVICE_REVOKED', 'revoked'); })}
      />,
    );
    await flush();
    expect(output.textContent).toContain('Phone access removed');
    expect(output.textContent).toContain('Pair again from your Mac to continue.');
  });
  ...
  it('shows the lost-key re-pair copy when vault restore fails closed', async () => {
    ...
    const vault: PhoneVaultPort = {
      restorePairedDevice: vi.fn(async () => { throw new BrowserVaultError('BROWSER_VAULT_KEY_LOST', 'lost'); }),
    };

    const output = await render(<PhonePairingScreen pairingClient={pairingClient} vault={vault} />);
    await flush();
    expect(output.textContent).toContain('This browser lost its secure device keys. Pair again from your Mac to continue.');
  });
  ...
  it('shows the lost-key re-pair copy when remote verification reports key loss', async () => {
    ...
    const output = await render(
      <PhonePairingScreen
        pairingClient={pairingClient}
        vault={vault}
        remoteSessionFactory={vi.fn(async () => { throw new RemoteSessionError('KEY_LOST', 'lost'); })}
      />,
    );
    await flush();
    expect(output.textContent).toContain('This browser lost its secure device keys. Pair again from your Mac to continue.');
...
describe('createDesktopPairingClient', () => {
  it('bootstraps desktop CSRF from the frozen Gateway route and never calls /internal/', async () => {
    ...
    const fetchImpl: typeof fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/api/desktop/pairing/status')) {
        return jsonResponse(joinStatus('join-1234567890abcdef1234567890abcdef', 'started_awaiting_desktop_approval'));
      }
    ...
    expect(requests.every((request) => !request.url.includes('/internal/'))).toBe(true);
    expect(requests[0]?.init?.method).toBe('GET');
    expect((requests[1]?.init?.headers as Record<string, string>)['X-Nomad-CSRF']).toBe('csrf_token_00000001');
  ...
  it('surfaces bootstrap failure as CSRF unavailable and does not attempt desktop POSTs', async () => {
    const fetchImpl: typeof fetch = vi.fn(async () => new Response('unavailable', {
      status: 503,
      headers: {
    ...
    expect(String(vi.mocked(fetchImpl).mock.calls[0]?.[0])).toBe('https://nomad.local/api/desktop/security');
  });
  ...
  it('refreshes desktop security once when the server rejects a stale CSRF token', async () => {
    ...
    const fetchImpl: typeof fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith('/api/desktop/security')) {
        ...
        return jsonResponse({
          schema: 'nomad.gateway.desktop-security.v1',
          csrf_token: securityCalls === 1 ? 'csrf_token_00000001' : 'csrf_token_00000002',
        });
      }
      if (url.endsWith('/api/desktop/devices/current')) {
        const csrf = (init?.headers as Record<string, string>)['X-Nomad-CSRF'];
    ...
    await expect(client.getCurrentDevice()).resolves.toEqual(currentDevice(false));
    expect(requests.map((request) => request.url)).toEqual([
      'https://nomad.local/api/desktop/security',
      'https://nomad.local/api/desktop/devices/current',
      'https://nomad.local/api/desktop/security',
    ...
    expect(((requests[1]?.init?.headers) as Record<string, string>)['X-Nomad-CSRF']).toBe('csrf_token_00000001');
    expect(((requests[3]?.init?.headers) as Record<string, string>)['X-Nomad-CSRF']).toBe('csrf_token_00000002');
  });
});
...
function currentDevice(paired: boolean): DesktopPairingCurrentDevice {
  return {
    ...
    principal_alias: 'remote-paired-device',
    paired,
    device: paired ? { device_alias: 'device-alpha-01', pairing_epoch: 7 } : null,
  };
}
...
function createdJoin(): DesktopPairingCreatedJoin {
  return {
    ...
    join_url: 'https://nomad.example/j/join-1234567890abcdef1234567890abcdef#pair_secret_token_0000000000000000',
  };
}

function joinStatus(joinId: string, state: DesktopPairingJoinStatus['state']): DesktopPairingJoinStatus {
  return {
    schema: 'nomad.m3e.pairing.status-response.v1',
    join_id: joinId,
    state,
    challenge_id: state === 'started_awaiting_desktop_approval' ? 'challenge-0000000000000001' : null,
    expected_epoch: state === 'started_awaiting_desktop_approval' ? 1 : null,
    comparison_code: state === 'started_awaiting_desktop_approval' ? '042913' : null,
    expires_at: '2099-01-01T00:02:00.000Z',
  };
}
  ...
  return {
    ...
    last_good_projection: view ? {
      ...
      capability: {
        ...
        snapshot_seq: view.state.lastAppliedSeq,
        snapshot_digest: view.state.expectedDigest ?? `sha256:${'a'.repeat(64)}`,
        next_command_seq: 1,
        issued_at: '2099-01-01T00:00:00.000Z',
